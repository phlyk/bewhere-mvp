...
You will communicate with subagent mainly through a progress file is <PROGRESS> markdown file. It shall list all tasks and will be updated by the subagent after it has picked and implemented a task. Beware additional tasks MIGHT appear at each iteration.

Then you will start the implementation loop and iterate in it until all tasks are finished.

You HAVE to start a subagent with the following prompt <SUBAGENT_PROMPT> (see below). The subagent is responsible to list all remaining tasks and pick the one that it thinks is the most important.
...
When you have finished the implementation of the task, you have to ensure the tests pass, and fix all potential issues until the implementation is complete.
